...
colnames(data)
class(data$Age)
apply(data, 2, FUN= class)  #are all numeric (apply:to know the type)
dim(data)
head(data)
summary(data)
...
k1 <- amap::Kmeans(data[,-1],centers=3, iter.max = 200,nstart = 1,  method = c("euclidean")) #data[,-1]: to remove cust_id
