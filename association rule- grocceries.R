...
arules::inspect(Groceries[1:6])
#Find Frequent Itemset
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5)) #min: two items max: 5 items
inspect(frequentItems[1:10])
frequentItems
inspect(frequentItems[10:100]) #minimum purchASE IS 100 TIMES
...
rules4 = apriori (data=Groceries, parameter=list (supp=0.002,conf = 0.8), appearance = list (default="lhs",rhs="whole milk"), control = list (verbose=T))
inspect(rules3[1:5])
inspect(rules3)
