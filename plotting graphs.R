...
women
plot(women)
plot(women, type='p', pch=19, col="blue")
plot(women, type='l')
plot(women, type='b')
...
quantile(women$height, seq(0,1,0.1))#every 10th percent
quantile(women$height, seq(0,1,0.01)) #percentile

stem(women$height) #stem plot
...
#correlation plot
pairs(women)
cor(women$height,women$weight) #corr: direction(+/-) and strength of relationship
...
pairs(mtcars[c('mpg','wt','hp')]) #only 3 colums are compared
